    ...
    def __init__(self, rect):
        ...
        for i in range(10):
            self.idle_frames.append(pygame.transform.scale(
                pygame.image.load(f"images/enemy/Idle ({i + 1}).png"), rect.size))

        self.hurt_index = 0
        self.hurt_frames = []
        for i in range(10):
            self.hurt_frames.append(pygame.transform.scale(
                pygame.image.load(f"images/enemy/Hurt ({i + 1}).png"), rect.size))

        self.dead_index = 0
        self.dead_frames =[]
        for i in range(10):
            self.dead_frames.append(pygame.transform.scale(
                pygame.image.load(f"images/enemy/Dead ({i + 1}).png"),rect.size))
